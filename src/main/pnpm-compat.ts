 ...
 *
 * 放在这里(而不是 plugins.ts)是因为**插件安装与内置运行环境安装都要用**:
 * - 插件侧写进 profile 的 pnpm-workspace.yaml(`allowBuilds`)并动态补漏;
 * - 内置环境侧写进 `<runtimeRoot>/dsh/pnpm-workspace.yaml` —— 少了它,
 *   全新机器的一键安装会在 `pnpm add @deepseek-ai/dsh` 这一步被
 *   `ERR_PNPM_IGNORED_BUILDS` 拦死(2026-09-18 实测;详见下面的注释)。
 */
