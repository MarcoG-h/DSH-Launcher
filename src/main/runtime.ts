 ...
 *
 * 为什么必须有:全新机器的一键安装会现装一份**最新版 pnpm**(`npm i -g pnpm` 不锁版本),
 * 而 pnpm 12 起只认 pnpm-workspace.yaml 里的 `allowBuilds`,`--config.strictDepBuilds=false`
 * 这类命令行绕过**已经失效**(2026-09-18 实测:pnpm 12.4.2 + 该 flag → `ERR_PNPM_IGNORED_BUILDS`;
 * 同一 flag 在 pnpm 11.21 上有效,所以老机器/维护者机器上复现不出来)。dsh 的核心依赖
 * (koffi / node-pty / protobufjs / @google/genai / @deepseek-ai/dsh-subprocess-local)
 * 都需要 postinstall,少了白名单这一步,新用户第一次「一键安装运行环境」100% 失败。
...
export async function installRuntime(): Promise<CmdResult> {
  ...
  // 必须先落白名单:pnpm 12 起命令行绕过已失效,没它这一步必被 ERR_PNPM_IGNORED_BUILDS 拦死。
