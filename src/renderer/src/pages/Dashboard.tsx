...
export function Dashboard(): JSX.Element {
  const { state, log, start, stop, restart, openUi } = useHarness()
    ...
    <div className="p-5 space-y-5">
      ...
      <div className="panel p-5">
          ...
          <div className="flex items-center gap-2">
            {!running ? (
              ...
              <button className="btn btn-danger" disabled={busy !== null} onClick={() => void doStop()}>
                <StopIcon /> {busy === 'stop' ? '停止中…' : '停止'}
              </button>
            )}
            <button className="btn btn-ghost" disabled={busy !== null || !running} onClick={() => void doRestart()}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-4 mt-5 pt-4 border-t" style={{ borderColor: 'var(--border)' }}>
          ...
          <Meta label="就绪" value={status === 'running' ? '✔ 是' : '—'} />
